...
const PurchesHandlerPc = ({ onNext, onSubmited }: { onNext: (incre: number) => void; onSubmited: () => void; }) => {
    // const p: productType;
    const ref = useRef<HTMLDivElement>(null)
    const [isLoading, setIsLoading] = useState(false)

    const dispatch = useDispatch()
    const { purchaseDts = [], isFocus, purchaseMt, supplier, _sum } = useSelector(selectPurchase);
    ...
    const totalSumBy = () => _.sumBy(purchaseDts, (purchase) => Number(purchase.qty) * Number(purchase.unit_price));
                ...
                <table className="colspan border-collapse w-full">
                    <thead className='sticky top-0 bg-accent z-10 h-fit w-full'>
                        <tr>
                            ...
                            <th className="purchesList border w-32" title='purches unit price' >pur_price</th>
                    ...
                    <tbody>
                        {
                            purchaseDts.map((purchaseDt, i) => (
                                <ProductTr isFocus={isFocus} dispatch={dispatch} onChange={(prodInfo) => {
                                    handleChangePurchaseInfo(prodInfo, i)
                                }} purchaseDt={purchaseDt} key={String(i + 100)} indexOfrow={i} />
                            ))
                        }
                        <ProductTr dispatch={dispatch} onChange={handlePushPurchaseInfo} indexOfrow={purchaseDts.length} />
                        <tr className='border'>
                        ...
                        <tr className='border'>
                            ...
                            <td className='text-right border px-2 py-1'>
                                <DatePicker value={purchaseMt?.pur_date ? new Date(purchaseMt?.pur_date) : undefined} onSelect={(date) => {
                                    dispatch(purchaseSlice.actions.handleSetPur_date(date?.toISOString()))
                                }} />
                            </td>
                        </tr>
                        <tr className='border'>
                            ...
                            <td className='text-right border px-2 py-1'>
                                <TextField fullWidth value={purchaseMt?.discount} size="small" label="discount:{'- ৳'}" type="number" onChange={e => {
                                    dispatch(purchaseSlice.actions.handleSetDiscount(Number(e.target.value || 0)))
                                }}
                        ...
                        <tr className='border'>
                            ...
                            <td className='text-right border px-2 py-1'>
                                <TextField fullWidth value={purchaseMt?.vat} size="small" label="vat:{'- ৳'}" type="number"
                                    onChange={e => {
                                        dispatch(purchaseSlice.actions.handleSetVat(Number(e.target.value || 0)))
                                    }}
                                    disabled={!(_sum || purchaseDts.length)} required />
                            </td>
                        </tr>
                        <tr className='border'>
                            ...
                            <td className='text-right border px-2 py-1'>
                                <TextField fullWidth value={purchaseMt?.paid_amt} size="small" label="paid_amt:{'- ৳'}" type="number"
                                    onChange={e => {
                                        dispatch(purchaseSlice.actions.handleSetPaid_amt(Number(e.target.value || 0)))
                                    }}
                                    disabled={!(_sum || purchaseDts.length)}
                                    required />
                            </td>
