...
import { productPostAsync, productSlice, selectProducts, useDispatch, useSelector } from "@/lib/redux";
...
const NewProduct = () => {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        try {
            dispatch(productPostAsync(values))
        } catch (error: any) {
            console.log({ error })
        }
    }
    ...
    return (
        <AlertDialog open={newState.open}>
            ...
            <AlertDialogContent className="overflow-auto max-h-[75vh] text-foreground">
                {newState.isLoading && <div className="w-full h-full absolute inset-0 animate-pulse bg-slate-500/30 bg-opacity-30 z-20 grid place-items-center">
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                        ...
                        <div className="space-y-5 md:gap-6">
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormControl>
                                            <TextField size="small" fullWidth autoFocus label="product name" placeholder="product name" {...field} required />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
                            ...
                            <div className="flex ">
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormControl>
                                                <TextField size="small" fullWidth label="product type" placeholder="product type" {...field} required />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                            ...
                            <div className="flex">
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormControl>
                                                <TextField size="small" fullWidth label="product uom" placeholder="product uom" {...field} required />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                        ...
                        {newState?.error && <p className="text-red-500 text-right">{newState.error}</p>}
                        <AlertDialogFooter>
