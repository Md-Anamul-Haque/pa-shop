...
const nextIndex = (purchaseDts: purchaseDetailType[], idx: number, key: string, isnext: boolean): { rowNumber: number, key: string } => {
    let returnValue = {
        rowNumber: idx,
        key: ''
    };
...
export const purchaseSlice = createSlice({
    ...
    reducers: {
        ...
        handleSetDiscount(state, action: { payload: number }) {
            state.purchaseMt = { ...state.purchaseMt, discount: Number(action.payload) }
