...
export const handlePurchaseGET = async (req: Request, res: Response) => {
    try {
        ...
        const mtData = await sql`SELECT * FROM sales_return_mt WHERE sales_r_id = ${sales_r_id} AND org_code = ${org_code}`
        const dtsData = await sql`SELECT * FROM sales_return_dt WHERE sales_r_id = ${sales_r_id} AND org_code = ${org_code}`
